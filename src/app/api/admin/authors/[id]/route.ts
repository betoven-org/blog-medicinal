...
import { db } from "@/db";
import { authors, posts, media } from "@/db/schema";
  ...
  try {
    ...
    if (body.name !== undefined) {
      ...
      const [slugConflict] = await db
        ...
        .from(authors)
        .where(eq(authors.slug, updateData.slug as string))
        .limit(1);
    ...
    if (body.avatarId !== undefined) updateData.avatarId = body.avatarId || null;
